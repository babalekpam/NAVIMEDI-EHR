...
interface HospitalPatientInsurance {
  id: string;
  tenantId: string;
  patientId: string;
...
export default function HospitalPatientManagement() {
  ...
  const [editingInsurance, setEditingInsurance] = useState<HospitalPatientInsurance | null>(null);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // Query for all hospital patients
  ...
  const { data: patientInsurance, isLoading: insuranceLoading } = useQuery({
    queryKey: ["/api/hospital-patient-insurance", selectedPatient?.id],
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Hospital Patient Management</h1>
      </div>
