...
export default function LabResults() {
  ...
  const loadLabResults = async () => {
    try {
      ...
      setError(err.message || t('common.failedToLoad', { data: t('labResults.title').toLowerCase() }));
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="p-6 max-w-2xl mx-auto">
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">{t('labResults.loadingLabResults')}</p>
          </div>
        ) : error ? (
          ...
          <div className="bg-white rounded-xl shadow-md p-8 text-center">
            ...
            <p className="text-gray-600">{t('labResults.noLabResults')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg text-gray-800" data-testid={`text-test-name-${index}`}>
                      {result.testName || t('labResults.unknownTest')}
                    </h3>
                    <div className="flex gap-2 mt-2">
                      ...
                      {result.priority && (
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getPriorityColor(result.priority)}`} data-testid={`priority-lab-result-${index}`}>
                          {translateStatus(result.priority, t)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {result.orderedDate && (
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm" data-testid={`text-ordered-date-${index}`}>
                        {t('labResults.ordered')}: {format(new Date(result.orderedDate), 'MMM dd, yyyy')}
                      </span>
                    </div>
                  )}
                  ...
                  {result.completedDate && (
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm" data-testid={`text-completed-date-${index}`}>
                        {t('labResults.completed')}: {format(new Date(result.completedDate), 'MMM dd, yyyy')}
                      </span>
                    </div>
                  )}
