...
export default function Prescriptions() {
  ...
  const loadPrescriptions = async () => {
    try {
      ...
      setError(err.message || t('common.failedToLoad', { data: t('prescriptions.title').toLowerCase() }));
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="p-6 max-w-2xl mx-auto">
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">{t('prescriptions.loadingPrescriptions')}</p>
          </div>
        ) : error ? (
              ...
              <div key={prescription.id || index} className="bg-white rounded-xl shadow-md p-6" data-testid={`card-prescription-${index}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg text-gray-800" data-testid={`text-medication-${index}`}>
                      {prescription.medicationName || t('prescriptions.unknownMedication')}
                    </h3>
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mt-2 ${getStatusColor(prescription.status)}`} data-testid={`status-prescription-${index}`}>
                      {translateStatus(prescription.status, t)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-600 mb-1">{t('prescriptions.dosage')}</p>
                      <p className="font-semibold text-sm" data-testid={`text-dosage-${index}`}>
                        {prescription.dosage || t('common.notAvailable')}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 mb-1">{t('prescriptions.frequency')}</p>
                      <p className="font-semibold text-sm" data-testid={`text-frequency-${index}`}>
                        {prescription.frequency || t('common.notAvailable')}
                      </p>
                    </div>
                  </div>
                  ...
                  {prescription.prescribedDate && (
                    <div className="flex items-center gap-2 text-gray-600 pt-2">
                      ...
                      <span className="text-sm" data-testid={`text-prescribed-date-${index}`}>
                        {t('prescriptions.prescribed')}: {format(new Date(prescription.prescribedDate), 'MMM dd, yyyy')}
                      </span>
                    </div>
                  )}
