...
export default function Profile() {
  ...
  const loadProfile = async () => {
    try {
      ...
      setError(err.message || t('common.failedToLoad', { data: t('profile.title').toLowerCase() }));
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="p-6 max-w-2xl mx-auto">
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-blue-400 p-8 text-white text-center">
            ...
            <p className="text-blue-100 mt-1" data-testid="text-mrn">{t('profile.mrn')}: {profile?.mrn}</p>
          </div>
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="text-sm text-gray-600">{t('profile.dateOfBirth')}</p>
                <p className="font-semibold" data-testid="text-dob">
                  {profile?.dateOfBirth ? format(new Date(profile.dateOfBirth), 'MMM dd, yyyy') : t('common.notAvailable')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="text-sm text-gray-600">{t('profile.bloodType')}</p>
                <p className="font-semibold" data-testid="text-blood-type">{profile?.bloodType || t('common.notAvailable')}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="font-semibold" data-testid="text-phone">{profile?.phone || t('common.notAvailable')}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="font-semibold" data-testid="text-email">{profile?.email || t('common.notAvailable')}</p>
              </div>
            </div>
